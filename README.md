...
const cacheStorageKey = 'check-demo-1.0' // 版本号，当想更新缓存资源（文件、数据等）
...
self.addEventListener('fetch', function (e) {
  ...
  // 图片请求过滤,此判断可根据实际需求条件进行判断
  if (reqUrl.indexOf('/api/attachment/findAliAttachmentById') !== -1) {
    e.respondWith(
...
```
   vue.congfig.js内添加 CopyWebpackPlugin（复制文件插件）
   
   plugins.push(
      // 复制 pwa 所需的文件到输出的地方
       new CopyWebpackPlugin([
          {
            from: 'sw.js', // sw.js文件的路径
            to: 'sw.js' // 注册service worker用到 navigator.serviceWorker.register('sw.js')
          }
       )
   )
